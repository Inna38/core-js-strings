...
function formatTime(minutes, seconds) {
  const dateMinutes = minutes.toString().padStart(2, '0');
  const dateSeconds = seconds.toString().padStart(2, '0');
  return `${dateMinutes}:${dateSeconds}`;
}
...
function reverseWords(str) {
  ...
  let revers = '';
  for (let i = 0; i < arr.length; i += 1) {
    revers = arr.reverse().join('');
  }
  const res = revers.split(' ').reverse().join(' ');
  return res;
}
